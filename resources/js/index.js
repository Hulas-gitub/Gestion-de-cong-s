...
function getCsrfToken() {
    ...
    if (!metaTag) {
        console.error('❌ Meta tag CSRF introuvable ! Assurez-vous d\'avoir : <meta name="csrf-token" content="{{ csrf_token() }}">');
        return '';
    }
    ...
    forgotPasswordForm.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            if (response.ok && data.success) {
                ...
                showMiniToast(data.message || 'Erreur lors de l\'envoi du lien', 'error');
            }
        } catch (error) {
...
if (loginForm) {
    loginForm.addEventListener('submit', async (e) => {
        ...
        try {
            // ✅ Récupération correcte du token CSRF
            const csrfToken = getCsrfToken();
            ...
            // Gérer les réponses non-JSON (redirections, erreurs serveur)
            let data;
            ...
            if (contentType && contentType.includes('application/json')) {
                ...
                // Si la réponse n'est pas JSON, c'est probablement une redirection ou une erreur
                throw new Error('Réponse invalide du serveur');
            }

            if (response.ok && data.success) {
                showMiniToast(data.message || 'Connexion réussie !', 'success');
                ...
                if (btnContent) btnContent.style.opacity = '1';
                if (btnLoader) btnLoader.style.display = 'none';
                showMiniToast(data.message || 'Identifiants incorrects', 'error');
            }
        } catch (error) {
            // Masquer le loader en cas d'erreur
            if (btnContent) btnContent.style.opacity = '1';
