    ...
    return `
        <div class="demand-item w-full flex flex-col md:flex-row items-start md:items-center justify-between bg-gray-50 dark:bg-gray-700/50 rounded-lg p-4" data-id="${request.id}">
            ...
            <div class="flex items-center space-x-2 mt-4 md:mt-0">
                ...
                ${showActions ? `
                    <button  class="text-indigo-600 hover:text-indigo-900 dark:text-indigo-400 dark:hover:text-indigo-300 mr-3"onclick="showConfirmModal('approve', ${request.id})">
                        <i class="fa fa-check"></i>
                    </button>
                    <button  class="text-indigo-600 hover:text-indigo-900 dark:text-indigo-400 dark:hover:text-indigo-300 mr-3"onclick="showConfirmModal('reject', ${request.id})">
                       title=" <i class="fas fa-times"></i>
                    </button>
                ` : ''}
                ${showRevalidate ? `
                    <button  class="text-red-600 hover:text-red-900 dark:text-red-400 dark:hover:text-red-300 mr-3"" onclick="revalidateRequest(${request.id})">
                        <i class="fas fa-sync-alt mr-1"></i>Revalider
                    </button>
                ` : ''}

                <button class="text-indigo-600 hover:text-indigo-900 dark:text-indigo-400 dark:hover:text-indigo-300 mr-3" onclick="showDetailsModal(${request.id})">
                    title="Voir détails"  <i class="fas fa-eye"></i>
                </button>
            </div>
        </div>
    `;
