...
async function editUser(userId) {
    try {
        ...
        if (data.success) {
            ...
            const isChef = user.role?.nom_role?.toLowerCase().includes('chef');
            const modalId = isChef ? 'chef-modal' : 'employe-modal';
...
function openEditModal(type, userData) {
    ...
    const prefix = type;
    document.getElementById(`${prefix}-matricule`).value = userData.matricule || '';
...
function populateRoleSelect(type, selectedId = null) {
    ...
    const filteredRoles = type === 'employe'
        ? roles.filter(r => r.nom_role.toLowerCase().includes('employe'))
        : roles.filter(r => r.nom_role.toLowerCase().includes('chef'));
...
function populateDepartementSelects() {
    ...
    if (chefSelect) {
        const chefs = users.filter(u => u.role?.nom_role?.toLowerCase().includes('chef'));
...
async function generateAndSetMatricule(type) {
    const roleSelect = document.getElementById(`${type}-role`);
    if (!roleSelect || !roleSelect.value) return;

    try {
        const response = await fetch(`${API_BASE_URL}/users/generate-matricule`, {
            method: 'POST',
            headers: {
                'Accept': 'application/json',
                'Content-Type': 'application/json',
...
async function handleUserSubmit(type) {
    const formData = {
        ...
        matricule: document.getElementById(`${type}-matricule`).value,
