...
class AdministrationControllers extends Controller
{
    ...
    public function index()
    {
        ...
        // Récupérer les employés (rôle "Employé")
        $employes = User::with(['role', 'departement'])
            ->whereHas('role', function($query) {
                $query->where('nom_role', 'Employé')
                      ->orWhere('nom_role', 'Employe')
                      ->orWhere('nom_role', 'employe');
            })
            ->where('id_user', '!=', auth()->id())
            ->orderBy('created_at', 'desc')
            ->paginate(10, ['*'], 'employes_page');

        // Récupérer les chefs de département
        $chefs = User::with(['role', 'departement'])
            ->whereHas('role', function($query) {
                $query->where('nom_role', 'LIKE', '%chef%')
                      ->orWhere('nom_role', 'LIKE', '%Chef%')
                      ->orWhere('nom_role', 'Manager');
            })
            ->where('id_user', '!=', auth()->id())
            ->orderBy('created_at', 'desc')
            ->paginate(10, ['*'], 'chefs_page');
        ...
        // Pour les selects des modals - tous les départements sans pagination
        $allDepartements = Departement::orderBy('nom_departement')->get();

        // Pour les selects des modals - tous les chefs potentiels
        $allChefs = User::whereHas('role', function($query) {
            $query->where('nom_role', 'LIKE', '%chef%')
                  ->orWhere('nom_role', 'LIKE', '%Chef%')
                  ->orWhere('nom_role', 'Manager');
        })->get();
    ...
    /**
     * Créer un nouvel utilisateur et envoyer email avec credentials
     */
    public function store(Request $request)
    {
        // Validation - Le téléphone est maintenant REQUIS pour le mot de passe
        $request->validate([
            'nom' => 'required|string|max:100',
            'prenom' => 'required|string|max:100',
            'email' => 'required|email|unique:users,email',
            'telephone' => 'required|string|max:20', // REQUIS maintenant
        ...
        try {
            ...
            if ($role) {
                if ($role->nom_role === 'Admin') {
                    $prefix = 'ADM';
                } elseif (stripos($role->nom_role, 'chef') !== false || stripos($role->nom_role, 'manager') !== false) {
                    $prefix = 'MGR';
                }
            }
            ...
            $defaultPassword = $request->telephone;

            $user = User::create([
                'nom' => $request->nom,
                'prenom' => $request->prenom,
                'email' => $request->email,
                'password' => Hash::make($defaultPassword), // Hash du téléphone
                ...
                'conges_pris' => 0,
                'actif' => true, // Actif directement car pas besoin de token d'activation
            ]);

            // Envoyer l'email avec les informations de connexion
            try {
                $emailSent = $this->mailService->envoyerCredentialsCompte(
                    $user->email,
                    $user->nom,
                    $user->prenom,
                    $user->telephone, // Le mot de passe par défaut
                    $user->matricule
                );

                if (!$emailSent) {
                    Log::warning('Échec envoi email credentials', [
                        'user_id' => $user->id_user,
                        'email' => $user->email
                    ]);
                }
            } catch (\Exception $mailException) {
                Log::error('Erreur envoi email credentials', [
            ...
            return response()->json([
                'success' => true,
                'message' => 'Utilisateur créé avec succès. Un email avec les informations de connexion a été envoyé à ' . $user->email,
                'user' => $user
            ], 201);
    ...
    /**
     * Renvoyer l'email avec les credentials (remplace l'ancien resendActivation)
     */
    public function resendCredentials($id)
    {
        try {
            ...
            if ($emailSent) {
                return response()->json([
                    'success' => true,
                    'message' => 'Email avec les credentials renvoyé avec succès à ' . $user->email
                ]);
            } else {
                return response()->json([
                    'success' => false,
    ...
    /**
     * Générer un nouveau matricule automatiquement (PAS UTILISÉ MAIS GARDÉ POUR COMPATIBILITÉ)
     */
    ...
    {
        try {
            $departement = Departement::with(['chefDepartement'])
                ->withCount('employes')
                ->findOrFail($id);
